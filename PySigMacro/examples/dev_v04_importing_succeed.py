#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-03-13 10:17:54 (ywatanabe)"
    ...
    def demo_with_csv_import(self, file_path="C:\\Temp\\SigmaPlot_CsvDemo.JNB"):
        ...
        try:
            ...
            if self.create_sample_csv(csv_path):
                # Import the CSV file using the new method
                success = self.import_csv_via_put_data(worksheet, csv_path)
                if success:
                    print("Successfully imported CSV data")
                else:
                    # Try the original method as fallback
                    success = self.import_csv_data(worksheet, csv_path)
                    if success:
                        print("Successfully imported CSV data using fallback method")
                    else:
                        print("Failed to import CSV data")

                # Try to create a plot with the new wizard method
                ...
                if success:
                    ...
                    if success:
                        print("Successfully created plot using fallback method")
                    else:
                        print("Failed to create plot")
            else:
