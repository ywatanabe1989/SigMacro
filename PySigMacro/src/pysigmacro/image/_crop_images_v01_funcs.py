#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-04-01 09:35:04 (ywatanabe)"
...
def add_margins(image, width=None, height=None, width_mm=None, height_mm=None):
    ...
    if width_mm is not None:
        width = _mm_to_pixels(width_mm, dpi)
    ...
    if not input_files:
        ...
        for input_path in input_files:
            try:
                ...
                # Crop the image
                cropped_img = _crop_image(input_path, margin=margin)
                ...
                dpi = _get_image_dpi(cropped_img)

                # Calculate actual dimensions for reporting
                width_px = cropped_img.width
                height_px = cropped_img.height
                # print(
                #     f"Adding margins to meet target dimensions: width={width_px}px, height={height_px}px (DPI: {dpi})"
                # )

                # Save the processed image
                _save_image(cropped_img, output_path)
                wait(
                    wait_condition_func=lambda: os.path.exists(output_path),
                ...
                if remove_inputs:
                    ...
                    wait(
                        wait_condition_func=lambda: not os.path.exists(input_path),
                        success_msg=f"Input image removed: {input_path}",
                        failure_msg=f"Failed to remove input image: {input_path}",
                        sleep_sec=0.5,
                        max_trials=10
                    )
